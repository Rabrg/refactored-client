...
public class Client extends GameShell {
	...
	public final boolean method66(int toLocalX, int toLocalY, int hash) {
			...
			if (type == 10 || type == 11 || type == 22) {
				...
				calculatePath(2, 0, sizeY, 0, Client.localPlayer.pathY[0], sizeX, face, toLocalY,
						Client.localPlayer.pathX[0], false, toLocalX);
			} else {
				calculatePath(2, rotation, 0, type + 1, Client.localPlayer.pathY[0], 0, 0, toLocalY,
						Client.localPlayer.pathX[0], false, toLocalX);
			}
			lastClickX = clickX;
			lastClickY = clickY;
			...
			try {
				...
				if (type >= 1 && type <= 100 || type >= 701 && type <= 800) {
					...
					if (type >= i_433_) {
						...
						} else if (friendsListWorlds[type] == Client.nodeId) {
						...
						for (int i_441_ = 0; i_441_ < 5; i_441_++) {
							if (characterEditColors[i_441_] != 0) {
								model.recolor(Client.anIntArrayArray1028[i_441_][0],
										Client.anIntArrayArray1028[i_441_][characterEditColors[i_441_]]);
								if (i_441_ == 1) {
									model.recolor(Client.anIntArray1229[0],
											Client.anIntArray1229[characterEditColors[i_441_]]);
								}
							}
						}
						model.createBones();
	...
	public final void method84(String username, String password, boolean loginType) {
		...
		try {
			...
			bufferedConnection = new BufferedConnection(this, openSocket(43594 + Client.portOffset));
										...
										if (i_639_ > Rasterizer.topX - 32 && i_639_ < Rasterizer.bottomX
											...
											if (imagergb != null) {
												...
												if (imagergb.maxWidth == 33 || widget_635_.itemAmounts[i_636_] != 1) {
													...
													fontSmall.drawString(Client.method43(-33245, i_647_), i_639_ + 1
															+ i_641_, i_640_ + 10 + i_642_, 0);
													fontSmall.drawString(Client.method43(-33245, i_647_), i_639_
								...
								for (int i_665_ = 0; i_665_ < widget_635_.width; i_665_++) {
									if (widget_635_.items[i_663_] > 0) {
										...
										if (itemdefinition.stackable || widget_635_.itemAmounts[i_663_] != 1) {
											string += " x" + Client.formatAmount(widget_635_.itemAmounts[i_663_]);
										}
